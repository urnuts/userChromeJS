 ...
 ['', '====浏览器全局===='],
      ...
      //  ['W-', '缩小', function(){ ucjsMouseGestures_helper.zoomIn(); } ], //按住右键向下滚动
       ...
       // ['L<R', '缩放重置', function(){ ucjsMouseGestures_helper.zoomReset(); } ],//按住右键再按左键
        ...
        ['RDL', '查看页面信息', function(event) {   BrowserPageInfo(); }],  //自定义
        ['LDR', '打开脚本文件夹', function(event) {FileUtils.getFile('UChrm',['SubScript']).launch();}],  //自定义



//打开指定文件夹/我的电脑/计算器，命令行/IE等 【来源于奶酪的QuickOpen.uc.js】
        //['LDR', '我的电脑', function(event) {var path ="..\\..\\explorer.exe";	var file = Components.classes["@mozilla.org/file/local;1"].createInstance(Components.interfaces.nsIFile);file.initWithPath(path.replace(/^\./, Components.classes["@mozilla.org/file/directory_service;1"].getService(Components.interfaces.nsIProperties).get("SysD", Components.interfaces.nsIFile).path));file.launch();}],
       ...
       //['', '聚焦到地址栏', function(event) {   openLocation(); }],  //自定义
        ...
        //['', '打开鼠标手势设置文件', function(event) {FileUtils.getFile('UChrm',['SubScript', 'MouseGestures2_e10s.uc.js']).launch();}],  //自定义

        ['DRD', '打开二维码', function(event) {FileUtils.getFile('UChrm',['Local','Sunbox_tool','qr-code', 'index.html']).launch();}],  //自定义
